...
class Landscape
	{
	public:
		Landscape( int inputN, int inputK );
		Landscape();
		~Landscape();
		...
		void Join( Landscape & LOne, Landscape & LTwo );
...
Landscape::Landscape( int initialN, int initialK )
{
	this->n = initialN;
	this->k = initialK;
...
void Landscape::Join( Landscape & LOne, Landscape & LTwo ) // relate two landscapes payoff structures so that decisions must be made with respect to both
 ...
 */
{
	...
	fillPartialPayoffs( pow(2, (LOne.k+k+1) ));
	i = 0; int mmm = 0;
/*	while (i < n){ // test loop
 *		mmm = 0;
 ...
 */
	...
	while (i < pow(2,(2*n)))
		...
		while (jj < n)
			{
			kk=0;
			while (kk < pow(2, LOne.k + k+ 1))  // this loop goes through each decision kk, goes to the payoff map, and adds up all the vectors with configs that match
				{
				temp = payoffs[jj][kk].GetConfig();
				temp2 = configuration;
		...
		// d[configuration]=payoffSum;
		d[i].SetFitness( d[i].GetFitness()/ (double) (2*n) ); // the number of vectors that matched should be equal to the total number (N)
		i++;
		}
	i=0;
...
int main()
{
	...
	cin >> n;
	srand ( time(NULL) );
	cout << "Select k (" << n-1 << " or less): ";
	cin >> k;
/*	Landscape * pLandscape = 0;
 *	while(i < totalruns)
 *	{
 *		pLandscape = new Landscape(n,k);
 *		Counter.Add( pLandscape->getNumPeaks() );
 *		delete pLandscape;
 *		i++;
 *	}
 */	Landscape LOne( n, k);
