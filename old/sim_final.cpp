...
void Multicounter::Write()
{
	...
	out << itsName << " - " << itsTotalRuns << " Ttl: " << itsDouble << "  Mean: " << this->Average( "double" ) << "  Std.Dev: " << this->StdDev() << endl;
	out.close();
}
...
{
	cumulativeFitness[0] = (cumulativeFitness[0] * (double) updatesCalled + LJoinFitness) / (double) updatesCalled;
	cumulativeFitness[1] = (cumulativeFitness[1] * (double) updatesCalled + LOneFitness) / (double) updatesCalled;
	cumulativeFitness[2] = (cumulativeFitness[2] * (double) updatesCalled + LTwoFitness) / (double) updatesCalled;
	updatesCalled++;
}
...
bool Entity::Move( string destination, Landscape & LJoined )
{
	...
	string previous = itsPayoff.GetConfig();
	if ( returnNextRound == 1 ) { LastConfig = previous; }
...
/* derived from a Landscape, also contains 2 landscapes */
{
	public:
		friend class Search;
		JoinedLandscape( int inputN, int inputK, int inputK_two, unsigned short int numEntities, char initialStrategy, string searchString );
		~JoinedLandscape() {}
		int WriteOut ();
...
JoinedLandscape::JoinedLandscape( int inputN, int inputK, int inputK_two, unsigned short int numEntities, char initialStrategy, string searchString ):
	itsK ( inputK ),
	itsN ( 2 * inputN ),
	itsK_two ( inputK_two),
...
void JoinedLandscape::AddEntity (string method, int number)
{
	...
	for (int i=0; i < number; i++)
	{
		...
		Entities.back().Move( target, *this ); // sets this newly-created entity's configuration to "target"
	}
}
...
class Search
{
	public:
		...
		string checkVicinityUnbalanced(JoinedLandscape & thisLandscape, Entity & thisEntity, bool Offline, int WhichLandscape);
	...
	CorpseCounter("Failed")
{
	...
	string temp, pattern;
	unsigned short int first=1, second=2, jj=0, strLength=0, counter=0; // parallel search by default
	bool Offline, Sequential;
	bool success;
	unsigned short int breakpoint = totalSearches / 2;
	vector <Entity>::iterator IT;
	/* ******************************** */
	...
	for ( IT = thisLandscape.Entities.begin() ; IT != thisLandscape.Entities.end() ; IT++ )
	{
		...
		switch ( (*IT).Strategy ) // this is where the magic happens, and searches are defined
		{
			...
			case 'F': pSearchFunc = &Search::checkVicinityBlind;
				  Offline = 1; Sequential = 1;
				  break;
			case 'G': pSearchFunc = &Search::checkVicinityBlind;
				  Offline = 0; Sequential = 1;
				  break;
			...
			case 'I': pSearchFunc = &Search::checkVicinityDist;
				  Offline = 0; Sequential = 1;
				  break;
			case 'J': pSearchFunc = &Search::checkVicinityUnbalanced;
				  Offline = 1; Sequential = 1;
				  break;
			case 'K': pSearchFunc = &Search::checkVicinityUnbalanced;
				  Offline = 0; Sequential = 1;
				  break;
		}
		if ( Offline )
	...
	if ( strategy == 'X' )
	{
	// better make this an iterator or loop or something
...
/* this version finds the move with the greatest improvement choices constant */
{
	...
	if ( Offline ){
		...
		if ( tempAverage <= tempPayoff.GetFitness() ) { thisEntity.returnNextRound = 1; } // if less, return next round
		if ( isEqual ( tempAverage, tempPayoff.GetFitness() ) ) { thisEntity.returnNextRound = 1; } // if equal, no reason to move
	}
...
string Search::checkVicinityUnbalanced( JoinedLandscape & thisLandscape, Entity & thisEntity, bool Offline, int WhichLandscape )
	...
	WriteToFile(0)
{
	...
	cout << endl << "A) Find greatest potential payoff (non-blind)"
	     << endl << "B) Offline parallel (blind)"
	     << endl << "C) Online parallel (blind)"
	     << endl << "D) Offline parallel by distribution (blind)"
	     << endl << "E) Online parallel by distribution (blind)" 
	     << endl << "F) Offline sequential (blind)"
	     << endl << "G) Online sequential (blind)"
	     << endl << "H) Offline sequential by distribution (blind)"
	     << endl << "I) Online sequential by distribution (blind)"
	     << endl << "J) Offline 'focused' (blind/sequential)"
	     << endl << "K) Online 'focused' (blind/sequential)"
...
*/	
	...
	for ( i = 0; i < walks ; i++ )
	{
		selectionStrength.push_back(.05);
	}
	...
	vector <Search> Searches;
 	for( i = 0; i < totalRuns; i++)
 	{
		...
		for ( j = 0 ; j < walks ; j++ )
		{
			Searches.push_back( Search( * pLandscape, searchChar, selectionStrength[j], j+1, walks )) ;
			// if( ((2*j) % walks) == 0 ) { pLandscape->AddEntity("O", 10); }
