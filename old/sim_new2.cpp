...
void Entity::ReturnToLast( Landscape & thisLandscape )
{
	if ( Alive == 1 ){ 
		...
		itsPayoff.SetFitness( thisLandscape.d[LastConfig].GetFitness() ) ;
...
/* derived from a Landscape, also contains 2 landscapes */
{
	public:
		...
		void AddEntity( string strategy, int number );
	private:
...
void JoinedLandscape::AddEntity (string strategy, int number)
{
	...
	{
		Entities.push_back( Entity( (int) itsN, * this, id++ ) );
...
{
	public:
		Search( JoinedLandscape & thisLandscape, char strategy, float strength, bool Offline );
		~Search() {};
...
Search::Search ( JoinedLandscape & thisLandscape, char strategy, float strength, bool isOffline ):
	...
	CorpseCounter("Failed")
{
	...
	for ( IT = thisLandscape.Entities.begin() ; IT != thisLandscape.Entities.end() ; IT++ )
	{
		...
		move = (*this.*pSearchFunc) ( thisLandscape , (*IT), Offline, 1 );
		success = (*IT).Move( move, thisLandscape );
		/* second search attempt */
		move = (*this.*pSearchFunc) ( thisLandscape , (*IT), Offline, 2 );
		success = (*IT).Move( move, thisLandscape );
	}
	/* ************************ */
	...
	totalEntities(200)
{
	...
	/* ************************************************* */
	cout << endl << "1) Find greatest potential payoff (non-blind)"
	     << endl << "2) Offline parallel (blind)"
	     << endl << "3) Offline parallel by distribution (blind)"
	...
	vector <Search> Searches;
 	for( i = 0; i < totalRuns; i++)
 	{
		...
		for ( j = 0 ; j < walks ; j++ )
		{
			Searches.push_back( Search( * pLandscape, searchChar, selectionStrength[j], Offline )) ;
