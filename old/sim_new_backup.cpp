 ...
 * 
 * i have multiple entities implemented but only the first search function works right
 * 
 * 
 */
...
void genConfig ( int X, string & config, int length);
	...
	key( inputN , vector <string> () )
{
	...
	unsigned short int ii = 0, jj = 0, ll = 0, choiceNum = 0, randomChoice = 0, counter = 0, binaryLength = itsK + 1;
...
void Landscape::sumPayoffs( unsigned short int size, int subN, int kOffset )
{
	...
	unsigned short int ii, jj, kk, success;
...
{
	public:
		Entity ( int size, Landscape & LJoined);
		Entity () {};
		~Entity () {};
...
Entity::Entity ( int size, Landscape & LJoined ):
	movesTaken(0),
	Alive(1),
	itsSize(size)
{
...
/* derived from a Landscape, also contains 2 landscapes */
{
	public:
		...
		unsigned short int GetDistribution ( string configuration, unsigned short int which );
	...
	LTwo( inputN, inputK, 0 )
{
	...
	{ 
		Entities.push_back( Entity( (int) itsN, * this ));
	}
}
...
{
	public:
		Search( JoinedLandscape & thisLandscape, char strategy );
		~Search() {};
...
Search::Search ( JoinedLandscape & thisLandscape, char strategy ):
	...
	totalEntities(50)
{
	...
	vector <Search> Searches;
 	for( i = 0; i < totalRuns; i++)
 	{
		...
		for ( j = 0 ; j < walks ; j++ )
		{
			Searches.push_back( Search( * pLandscape, searchChar )) ;
		}
...
void genConfig(int X, string & binaryConfig, int length)
{
	binaryConfig.clear();
	if (length > 0)
	{
