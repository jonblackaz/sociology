...
class Entity
	...
	public:
		...
		~Entity ();
		Payoff checkVicinity_one( map< string , Payoff > & d, map< string , Payoff > & d_two, unsigned short int maxN);
		Payoff checkVicinity_all( map< string , Payoff > & d, map< string , Payoff > & d_two);
		bool Move( Payoff destination );
		int idNeighbors();
	private:
		int itsN;
		Payoff itsPayoff;
...
Payoff Entity::checkVicinity_one( map< string , Payoff > & d, map< string , Payoff > & d_two, unsigned short int maxN)
/* this version keeps landscape 2 choices constant */
{
	...
	for ( i = 0 ; i < maxN ; i++)
	{
		...
		cout << tempConfig << ":" << tempConfig_two << ":" << tempAverage << endl;
	...
	if ( Move.GetFitness() <= itsPayoff.GetFitness() ) { return Nil; }
	return Move;
}

/*Payoff Entity::checkVicinity_one( map< string , Payoff > & d, map< string , Payoff > & d_two, unsigned short int maxN, unsigned short int k )
 this version keeps landscape 2 choices constant ***BROKEN CURRENTLY*** actually, it magically selects any config in landscape 2
{
	Payoff Move;
	Payoff Nil( "EMPTY" , 0 );
	string tempConfig = itsPayoff.GetConfig(), tempBinary, reallytempConfig;
	unsigned short int i, j;
	unsigned short int strLength;
	vector <Payoff> averages;
	double tempAverage;
	...
	for ( i = 0 ; i < maxN ; i++)
	{
		...
		for ( j = 0 ; j < pow (2,maxN) ; j++ )
			{
			reallytempConfig.clear();
			tempBinary.clear();
			reallytempConfig = tempConfig;
			binary( j , tempBinary );
			strLength = tempBinary.length();
			reallytempConfig.replace( maxN , strLength , tempBinary );
			cout << tempConfig << ":" << reallytempConfig << endl;		
			tempAverage = (d[reallytempConfig].GetFitness() + d_two[reallytempConfig].GetFitness()) / (double) 2;
			averages.push_back ( Payoff(reallytempConfig,tempAverage) );
			}
	}
	Move.SetFitness(0);
	vector <Payoff>::iterator IT;
...
Payoff Entity::checkVicinity_all( map< string , Payoff > & d, map< string , Payoff > & d_two)
...
bool Entity::Move( Payoff Move )
{
	if ( Move.GetConfig() == "EMPTY" ) { return 0; }
	cout << "Move from " << itsPayoff.GetConfig() << ":" << itsPayoff.GetFitness() << " to ";
	itsPayoff.SetFitness( Move.GetFitness() );
	itsPayoff.SetConfig( Move.GetConfig() );
...
int main()
{
	...
	Multicounter Counter, Counter_two;
	cout << "How many runs? ";
	cin >> totalruns;
	cout << "Select a size: ";
	cin >> n;
	...
	cin >> walks;
	cout << endl << "a) Keeps landscape 2 constant"
	     << endl << "b) Optimizes all choices at the same time" << endl;
	cout << "Select a search strategy[default='a']: " ;
	cin >> search_strategy;
	...
	bool success = 0;
 	while( i < totalruns)
 	{
		...
		pLandscape_join = new JoinedLandscape(*pLandscape, *pLandscape_two, k_two);
 		...
 		Counter_two.Add( pLandscape_join->getNumPeaks2() );
		...
		if ( search_strategy == "a")
		{
			for ( j = 0 ; j < walks ; j++)
			{
				...
				success = pEntity->Move(move);
				if ( !success ) { cout << "No move (1)" << endl; }
			}
			for ( j = 0 ; j < walks ; j++)
			{
				move = pEntity->checkVicinity_one( pLandscape_join->d_two , pLandscape_join->d , n );
				success = pEntity->Move(move);
...
//	cout << Counter_two.Average( "int" ) << endl;
	return 0;
}
