...
bool Entity::Move( string Move, map< string , Payoff > & d, map< string , Payoff > & d_two )
{
	...
	cout << itsPayoff.GetConfig() << " " << Move << endl;
...
int main()
{
	unsigned short int n=0, k=0, totalruns=0, i=0, k_two=0, walks=0, j=0, L=0;
	unsigned short int totalEntities = 50;
	unsigned short int search_strategy;
	Multicounter CounterA("A"), CounterB("B"), CounterC("C"), CounterD("D"), CounterE("E");
	cout << "How many runs? ";
	cin >> totalruns;
	cout << "Select a size: ";
	cin >> n;
	...
	cin >> search_strategy;
	if ( (search_strategy > 5) || (search_strategy < 1) )
		{
		search_strategy = 9;
		}
	...
	bool success = 0;
 	...
 	while( i < totalruns)
 	{
		...
		while ( L < totalEntities ) { Entities.push_back ( Entity( pLandscape_join->d, pLandscape_join->d_two, 2*n) ); L++; }
		...
		for ( IT = Entities.begin() ; IT != Entities.end() ; IT++ )
		{
			...
			if ( search_strategy == 1 || search_strategy == 9)
			{
				...
				for ( j = 0 ; j < walks ; j++) // success is not cleared?
				{
