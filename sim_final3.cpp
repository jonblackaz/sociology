...
class Entity
{
	...
	private:
		...
		string LastConfigOne;
		string LastConfigTwo;
		double LastFitnessOne;
		double LastFitnessTwo;
		Payoff itsPayoff;
...
Entity::Entity ( int size, Landscape & LJoined, unsigned int ID, char itsStrategy ):
	returnNextRound[0](0), // [0] means first subperiod
	returnNextRound[1](0), // [1] means second subperiod
	movesTaken(0),
	Alive(1),
	itsSize(size),
	itsID(ID),
...
void Entity::ReturnToLast( Landscape & thisLandscape )
{
	if ( Alive == 1 ){ 
		...
		if ( returnNextRound[1] == 1 ){
			// cout << "Move from " << itsPayoff.GetConfig() << ":" << itsPayoff.GetFitness() ;
			itsPayoff.SetConfig( LastConfigOne );
			itsPayoff.SetFitness( thisLandscape.GetFitness(LastConfigOne) ) ;
			// cout << "to " << itsPayoff.GetConfig() << ":" << itsPayoff.GetFitness() << endl;
			thisLandscape.AddToDistribution( LastConfigOne );
		}
		
		if (returnNextRound[0] ==1 ){
			// cout << "Move from " << itsPayoff.GetConfig() << ":" << itsPayoff.GetFitness() ;
			itsPayoff.SetConfig( LastConfigTwo );
			itsPayoff.SetFitness( thisLandscape.GetFitness(LastConfigTwo) ) ;
			// cout << "to " << itsPayoff.GetConfig() << ":" << itsPayoff.GetFitness() << endl;
			thisLandscape.AddToDistribution( LastConfigTwo );
		}
...
bool Entity::Move( string destination, Landscape & LJoined, int searchPeriod )
{
	...
	// Don't move those with no direction.
	...
	// Remove these from the running tallies.
	...
	// Telling the entity that it has moved.
	itsPayoff.SetConfig( destination );
	itsPayoff.SetFitness( LJoined.d[destination].GetFitness() ) ;
	// cout << "to " << itsPayoff.GetConfig() << ":" << itsPayoff.GetFitness() << endl;

	// Adding it to the running tally.
	LJoined.AddToDistribution( destination );

	// How far has it gone?
	movesTaken++;
	
	// More keeping track of where it was.
	if ( searchPeriod == 1 )
	{
		LastConfigOne = previous;
		LastFitnessOne = previousFitness;
	}
	if ( searchPeriod == 2 )
	{
		LastConfigTwo = previous;
		LastFitnessTwo = previousFitness;
	}

	// Now, if this is move number two, figure what configuration we're supposed to return to, if any.
	if ( searchPeriod = 2 )
	{
		(( LastFitnessTwo < itsPayoff.GetFitness()) && !isEqual(LastFitnessTwo, itsPayoff.GetFitness() )) // if this second move was good
		? (
			(( LastFitnessOne < itsPayoff.GetFitness() ) && !isEqual(LastFItnessOne, itsPayoff.GetFitness() )) // and so was the first move 
			? return 1	// everything is happy, return
			: returnNextRound[0] = 1 // the current fitness is less than it was before the first move, so go back to the beginning
		  )
		: ( 
	...
	ChoiceDistributions(thisLandscape.GetN(), 0.5)
{
	...
	for ( IT = thisLandscape.Entities.begin() ; IT != thisLandscape.Entities.end() ; IT++ )
	{
		...
		if ( !Offline )
		{
			if( (*IT).returnNextRound[0] == 1 || (*IT).returnNextRound[1] == 1 ) { (*IT).ReturnToLast( thisLandscape ); }
