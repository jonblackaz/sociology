...
// searches. It's not a search in and of itself.
{
	...
	unsigned short int whichLandscape = thisLandscape.getId();
	...
	if ( whichLandscape == 1 )
	{
		...
		else
		{
			...
			if ( (period <= itsStrategy.GetBreakpoint() ) && (itsStrategy.isSequential()) )
			{
...
Search::Search ( JoinedLandscape & thisLandscape, char strategy, float strength, unsigned short int period, unsigned short int totalSearches):
	...
	ChoiceDistributions(thisLandscape.GetN(), 0.5)
{
	...
	if (thisPeriod == (breakpoint + 1))
	{
		sequentialChange = true;
		cout << "breakpoint: " << period-1 << endl;
	}
	for ( IT = thisLandscape.Entities.begin() ; IT != thisLandscape.Entities.end() ; IT++ )
	{
		first=1; second=2;
		switch ( (*IT).itsStrategy.isCalled() ) // this is where the magic happens, and searches are defined
		{
...
/* randomly selects one move */
{
	...
	i = thisEntity.itsMoves.GetPossible( ( *pLandscape), thisEntity.GetConfig(), thisPeriod, thisSubperiod );
							      //// REMEMBER, THE WHOLE REASON YOU ARE DOING THIS IS
							     //// BECAUSE THE PROPER LANDSCAPE ISN'T BEING USED
							    //// TO COMPARE WHETHER TO MOVE OR NOT (OR RETURN)
	// cout << "i: " << i << endl;
	if ( i < 0 ) { return "EMPTY"; }
